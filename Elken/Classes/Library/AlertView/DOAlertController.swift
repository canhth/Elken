...
class DOAlertController : UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
    ...
    var alertViewBgColor = UIColor(hex: "345D80")
    fileprivate var alertViewWidth: CGFloat = 270.0 * RATIO.SCREEN_WIDTH
    fileprivate var alertViewHeightConstraint: NSLayoutConstraint!
    fileprivate var alertViewPadding: CGFloat = 25.0
    fileprivate var innerContentWidth: CGFloat = 240.0 * RATIO.SCREEN_WIDTH
    ...
    var titleFont = UIFont.setFontNameWithSize(size: 17, isBold: true)
    var titleTextColor = UIColor.white
    
    // MessageView
    fileprivate var messageView = UILabel()
    var messageFont = UIFont.setFontNameWithSize(size: 12, isBold: false)
    var messageTextColor = UIColor.white
    
    // TextFieldContainerView
    fileprivate var textFieldContainerView = UIView()
    var textFieldBorderColor = UIColor(red: 205.0/255, green: 205.0/255, blue: 205.0/255, alpha: 1.0)
    
    // TextFields
    fileprivate(set) var textFields: [AnyObject]?
    fileprivate let textFieldHeight: CGFloat = 30.0
    var textFieldBgColor = UIColor.white
    fileprivate let textFieldCornerRadius: CGFloat = 15 * Screen.WIDTH / 320
    ...
    fileprivate let buttonHeight: CGFloat = 35 * RATIO.SCREEN_HEIGHT
    ...
    var buttonFont: [DOAlertActionStyle : UIFont?] = [
        .default : UIFont.setFontNameWithSize(size: 16, isBold: true),
        .cancel  : UIFont.setFontNameWithSize(size: 14, isBold: true),
        .destructive  : UIFont.setFontNameWithSize(size: 14, isBold: true)
    ]
    ...
    var buttonBgColorHighlighted: [DOAlertActionStyle : UIColor] = [
        ...
        .cancel  : UIColor(red:255/255, green:255/255, blue:255/255, alpha:1),
        .destructive  : UIColor(red:234/255, green:97/255, blue:83/255, alpha:1)
    ]
    fileprivate var buttonCornerRadius: CGFloat = 20 * Screen.WIDTH / 320
    ...
    convenience init(title: String?, message: String?, preferredStyle: DOAlertControllerStyle) {
        ...
        buttonAreaScrollView.bounces = false
        textAreaScrollView.bounces = false
        // NotificationCenter
        ...
        // AlertView
        alertView.layer.cornerRadius = 5 * RATIO.SCREEN_WIDTH
    ...
    func layoutView() {
        ...
        if (hasTitle) {
            ...
            titleLabel.frame = CGRect(x: 0, y: textAreaPositionY, width: innerContentWidth, height: titleLabel.frame.height + 10)
            textContainer.addSubview(titleLabel)
            textAreaPositionY += titleLabel.frame.height + 15.0
        }
        ...
        if (isAlert() && buttons.count == 2) {
            ...
            for button in buttons {
                ...
                button.backgroundColor = buttonBgColor[action.style]!
                ...
                if button.titleLabel?.text != "Cancel" && button.titleLabel?.text != "No"
                {
                ...
                else
                {
                    ...
                    button.layer.borderColor = textFieldBorderColor.cgColor
                }
        ...
        else if (isAlert() && buttons.count == 1)
        {
            ...
            if (action.style != DOAlertActionStyle.cancel) {
                ...
                button.setTitleColor(buttonTextColor[action.style], for: UIControlState())
                button.backgroundColor = buttonBgColor[action.style]!
                button.frame = CGRect(x: alertViewWidth / 2 - innerContentWidth / 4 - buttonMargin, y: buttonAreaPositionY, width: innerContentWidth / 2, height: buttonHeight)
        ...
        // UnderLine view
        if isHasUnderLine {
            let underLineView = UIView(frame: CGRect(x: 0, y: buttonAreaScrollView.frame.origin.y + 1, width: alertViewWidth, height: 1))
            underLineView.backgroundColor = UIColor.lightGray
            buttonAreaScrollView.addSubview(underLineView)
        }
    }
    ...
    func reloadAlertViewHeight() {
        ...
        var alertViewHeight = textAreaHeight + buttonAreaHeight + 20
