...
public struct SlideMenuOptions {
    public static var leftViewScale: CGFloat = 0.8
    ...
    public static var rightViewScale: CGFloat = 0.8
...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    public convenience init(mainViewController: UIViewController, leftMenuViewController: UIViewController) {
        ...
        mf_swapLeftAndRightControllerIfIsRTLLanguage()
        initView()
    }
    ...
    open func initView() {
      ...
      if leftViewController != nil {
        ...
        leftFrame.size.width = SlideMenuOptions.leftViewScale * view.bounds.width
      ...
      if rightViewController != nil {
        ...
        rightFrame.size.width = SlideMenuOptions.rightViewScale * view.bounds.width
    ...
    open override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: nil, completion: { [weak self] (context: UIViewControllerTransitionCoordinatorContext!) -> Void in
            if let strongSelf = self {
                strongSelf.closeLeftNonAnimation()
                strongSelf.closeRightNonAnimation()
                strongSelf.leftContainerView.isHidden = false
                strongSelf.rightContainerView.isHidden = false
                
                if strongSelf.leftPanGesture != nil && strongSelf.leftPanGesture != nil {
                    strongSelf.removeLeftGestures()
                    strongSelf.addLeftGestures()
                }
                
                if strongSelf.rightPanGesture != nil && strongSelf.rightPanGesture != nil {
    ...
    open func addLeftGestures() {
        ...
        if (leftViewController != nil) {
            ...
            if leftTapGesture == nil {
                leftTapGesture = UITapGestureRecognizer(target: self, action: #selector(SlideMenuController.toggleLeft))
    ...
    fileprivate func addRightGestures() {
        ...
        if (rightViewController != nil) {
            ...
            if rightTapGesture == nil {
                rightTapGesture = UITapGestureRecognizer(target: self, action: #selector(SlideMenuController.toggleRight))
        ...
        switch panGesture.state {
            ...
            case UIGestureRecognizerState.ended, UIGestureRecognizerState.cancelled:
                if LeftPanState.lastState != .changed {
                    // NOTE: DuyPhan: force close slide menu
                    setCloseWindowLebel()
                    track(.flickClose)
                    return
                }
    ...
    open func changeLeftViewWidth(_ width: CGFloat) {
        SlideMenuOptions.leftViewScale = width / view.bounds.width
        var leftFrame: CGRect = view.bounds
    ...
    open func changeRightViewWidth(_ width: CGFloat) {
        SlideMenuOptions.rightViewScale = width / view.bounds.width
        var rightFrame: CGRect = view.bounds
    ...
    fileprivate func leftMinOrigin() -> CGFloat {
        return  -SlideMenuOptions.leftViewScale * view.bounds.width
    }
...
extension SlideMenuController {
    ...
    fileprivate func closeLeftWithVelocity(_ velocity: CGFloat, keepShadowUntilCompletion completion: (() -> Void)?) {
        let xOrigin: CGFloat = leftContainerView.frame.origin.x
        let finalXOrigin: CGFloat = leftMinOrigin()
    ...
    fileprivate func closeRightWithVelocity(_ velocity: CGFloat, keepShadowUntilCompletion completion: (() -> Void)?) {
    ...
    fileprivate func closeLeftWithCompletion(_ completion: (() -> Void)? = nil) {
        leftViewController?.beginAppearanceTransition(isLeftHidden(), animated: true)
        closeLeftWithVelocity(0.0, keepShadowUntilCompletion: {
            smc_delay(0.4) {
                // tnthuyen: It's kind of tricky: delay a little bit
                // so that the completion executes first
                self.hideShadow()
            }
            completion?()
        })
    ...
    fileprivate func closeRightWithCompletion(_ completion: (() -> Void)? = nil) {
        rightViewController?.beginAppearanceTransition(isRightHidden(), animated: true)
        closeRightWithVelocity(0.0, keepShadowUntilCompletion: {
            smc_delay(0.4) {
                // tnthuyen: It's kind of tricky: delay a little bit
                // so that the completion executes first
                self.hideShadow()
    ...
    fileprivate func mf_swapLeftAndRightControllerIfIsRTLLanguage() {
        if !isRTLLanguage { return }
        let tempVC = leftViewController
        leftViewController = rightViewController
        rightViewController = tempVC
    }
    
    fileprivate func hideShadow() {
        self.opacityView.layer.opacity = 0.0
        self.removeShadow(self.leftContainerView)
    }
    
    public func mf_toggle() {
        if leftViewController != nil {
            toggleLeft()
        } else {
            toggleRight()
    ...
    public func mf_closeWithCompletion(_ completion: (() -> Void)? = nil) {
        if leftViewController != nil {
            closeLeftWithCompletion(completion)
        } else {
            closeRightWithCompletion(completion)
        }
    }
...
public extension UIViewController {
    ...
    public var isRTLLanguage: Bool { return view.isRightToLeftLanguage }
...
func smc_delay(_ delay:Double, closure:@escaping ()->()) {
    DispatchQueue.main.asyncAfter(
        deadline: DispatchTime.now() + Double(Int64(delay * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC), execute: closure)
}

// MARK:- Fix bug update frame after rotation ios device
extension SlideMenuController {
    public func reloadSlideMenu() {
        // reload frame LeftView
